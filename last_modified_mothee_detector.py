...
def audio_deep_fake_detector_model():
    ...
    model_name_or_path = "motheecreator/Deepfake-audio-detection"
    model = AutoModelForAudioClassification.from_pretrained(model_name_or_path)
    feature_extractor = AutoFeatureExtractor.from_pretrained(model_name_or_path)
    return model , feature_extractor
speaker_model = load_model()
deep_fake_detector_model , feature_extractor = audio_deep_fake_detector_model()
...
def detect_deep_fake_audio(audio_path):
    ...
    # Add batch dimension back before passing to the model
    input_values = input_values.unsqueeze(0)
    # Perform inference
    with torch.no_grad():
        outputs = deep_fake_detector_model(**inputs)
        predictions = torch.nn.functional.softmax(outputs.logits, dim=-1)
        predicted_class = predictions.argmax().item()
    # Print the prediction
    labels = ["Deepfake" , "Real"]
    st.write(f"The audio is predicted to be: {labels[predicted_class]}")
