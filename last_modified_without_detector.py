...
def record_audio(max_duration=10):
    ...
    st.write("Recording... (up to 10 seconds)")
    recording = []
...
with tab3:
    ...
    enroll_option = st.selectbox("Choose an option for enrollment", ("Upload Audio", "Record Unlimited Audio", "Record Using Specific Duration" , "Record For 10 Seconds"))
    if user_id_exists(user_id):
        ...
        if enroll_option == "Record For 10 Seconds":
            if st.button("Start Recording for Enrollment"):
                recording, samplerate = record_audio()
                save_wav('enroll10sec.wav', recording, samplerate)
                embeddings = extract_embeddings('enroll10sec.wav')
...
with tab4:
    ...
    verify_option = st.selectbox("Choose an option for verification after enrollment", ("Upload Audio", "Record Unlimited Audio", "Record Using Specific Duration" , "Record For 10 Seconds"))
    ...
    if verify_option == "Record For 10 Seconds":
        if user_id_exists(user_id):
            if st.button("Start Recording for Verification After Enrollment"):
                recording, samplerate = record_audio()
                save_wav('verify_after_enrollment_10_seconds.wav', recording, samplerate)
                new_embeddings = extract_embeddings('verify_after_enrollment_10_seconds.wav')
                similarity, is_same_speaker = verify_user(user_id, new_embeddings)
                number = similarity
                percentage = number * 1000
                formatted_percentage = f"{percentage:.2f}%"
    ...
    if verify_option == "Record Using Specific Duration":
        if user_id_exists(user_id):
            ...
            if st.button("Start Recording for Verification After Enrollment"):
                recording, samplerate = record_audio_with_duration(duration)
                save_wav('verify_after_enrollment_specific_duration.wav', recording, samplerate)
                new_embeddings = extract_embeddings('verify_after_enrollment_specific_duration.wav')
                similarity, is_same_speaker = verify_user(user_id, new_embeddings)
                number = similarity
                percentage = number * 1000
                formatted_percentage = f"{percentage:.2f}%"
